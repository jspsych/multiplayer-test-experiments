...
| --- | --- | --- | --- | --- |
| A1 | Which experiment runs first | **Shortened Hawkins-style pilot**, not C&WG | The first paid run now prioritizes proving the multiplayer paid-run pipeline for the Hawkins-style task. Use 3 blocks / 36 rounds, not the full 6-block / 72-round replication (§5) | Medium — changing back reorients docs, cost assumptions, preregistration language, and branch base |
| A2 | Where phase-1 work lands | **A Hawkins paid-pilot branch built from PR #19 (`hawkins-cued-fidelity`)** | PR #19 contains the Hawkins fidelity fixes; the C&WG stack should be ported as infrastructure, not treated as the launch target | Medium — the longer infrastructure diverges between files, the more careful the port gets |
...
| A4 | DataPipe save shape | **Incremental chunks**, nonce-suffixed filenames; exact Hawkins cadence to decide during the port | OSF 409 on a duplicate filename is the one failure path that is *not* retried, so a collision loses that chunk permanently. Per-round saving was natural for 6-round C&WG; a 36-round Hawkins pilot should review save count versus data-loss risk (§3, #3) | Medium — changes analysis-side reassembly |
...
| # | Decision | Suggested | Why | Reversal cost |
| --- | --- | --- | --- | --- |
| B1 | Participant pay rate | **$15.00/hr** (placeholder: $7.50 per 30-min Hawkins pilot, $9.98 all-in) | Above Prolific's $12.00/hr recommendation. Sustained real-time coordination, no pausing, and attention *is* the measurement. Final reward must be set from measured duration of the 3-block pilot plus expected lobby wait (§5) | **Cannot change** for a published study |
...
| B4 | `limitSessions` | **Off** | DataPipe's counter increments per *save call*, not per participant. A naive per-round 36-round Hawkins save shape uses ~37 sessions per participant, so sizing the cap to participants would drop later saves silently (§3) | Medium — but the failure it prevents is silent and mid-run |
