 ...
 *     reference-game-hawkins.html  tangrams.js  hawkins.js
 *     assets/tangrams/...  vendor/...   (vendor/ as filled by scripts/vendor.mjs)
 *
...
const BATCH = {
  ...
  // The group size. The adapter seals a group once it has this many members, and the experiment
  // refuses to start unless it is 2.
  maxActiveMembers: 2,
  // Left unlimited: it counts members who left, so a cap of 2 would stop a group whose first
  // arrival left while it was forming from ever filling. Sealing already keeps newcomers out.
...
const OUT_NAME = process.env.JZIP_OUT ?? "reference-game-hawkins-jatos-prototype.jzip";

// Study files that ship as JATOS assets (jatos.js is server-injected). The vendor/ files are
// whatever scripts/vendor.mjs recorded in vendor/manifest.json.
const VENDOR_MANIFEST = path.join(REPO, "vendor", "manifest.json");
if (!fs.existsSync(VENDOR_MANIFEST)) {
  console.error("vendor/manifest.json is missing: run `node scripts/vendor.mjs` first");
  process.exit(1);
}
const vendorFiles = Object.keys(JSON.parse(fs.readFileSync(VENDOR_MANIFEST, "utf8")).files);
const ASSET_PATHS = [
  ...
  return JSON.stringify(
    {
      ...
      data: {
        ...
        title: STUDY_TITLE,
        description:
          "Hawkins reference-game paid pilot (36 rounds, 3 blocks). Paid-run validation build.",
        groupStudy: true,
        groupSessionWriteScope: "SHARED", // multiplayer slot writes span members
        linearStudy: false,
        allowPreview: false,
        dirName: DIR_NAME,
        comments: `Packaged by scripts/make-jzip.mjs. Batch caps: ${BATCH.maxTotalWorkers} total workers, ${BATCH.maxActiveMembers} active members per group.`,
        componentList: [
          {
            uuid: crypto.randomUUID(),
...
const outPath = path.resolve(REPO, OUT_NAME);
...
console.log(`batch: "${BATCH.title}" active, workers<=${BATCH.maxTotalWorkers}, group size ${BATCH.maxActiveMembers}`);
