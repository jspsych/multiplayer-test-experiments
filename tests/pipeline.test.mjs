// Tests for the DataPipe egress module (#3) in reference-game-hawkins.html.
//
...
if (!scriptMatch) throw new Error("Could not find the trailing <script> block in reference-game-hawkins.html");
const pipelineMatch = scriptMatch[1].match(/const Pipeline = \(\(\) => \{[\s\S]*?\n    \}\)\(\);/);
if (!pipelineMatch) throw new Error("Could not find the Pipeline module in reference-game-hawkins.html");
