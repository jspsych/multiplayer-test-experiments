// Tests for dropout detection (#5) in reference-game-hawkins.html.
//
...
// --- false positives: the expensive direction ------------------------------------------------
{
  ...
  Array.from({ length: 36 }, (_, i) => i + 1).forEach((r) => run(submitted(r)));
  check("a clean 36-round pilot never aborts", state.partnerDropped === false && state.aborted === 0);
}
{
  const { run, state } = makeDetector();
  // A genuine timeout with partner chat is a slow round, not evidence of dropout.
  run(slowRound(1));
  run(slowRound(2));
  run(slowRound(3));
