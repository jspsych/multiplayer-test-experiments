 ...
 * `jspsych-multiplayer` checkout. Mirrors the return-shape contract of `connectLocalDeployment()` in
 * `reference-game-hawkins.html` so the timeline never inspects an
 * adapter-specific API: `{ sessionId, allocatedParticipantIds,
 * finalizeAllocation }`, plus an optional `bootstrapError` on the
 * missing-group path.
 *
 * Stock JATOS presence is not an allocated roster, so both roster fields
 * are always `null`; the timeline's exactly-two roster validation remains
 * the safety gate. (A later part must make `finalizeAllocationTrial`
 * tolerate `finalizeAllocation: null` before this bootstrap is wired in.)
 *
