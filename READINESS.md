...
`chore/vendor-multiplayer-bundles`. That branch vendors the unpublished/incompatible multiplayer
bundles and remains the runnable-baseline packet; this documentation branch can merge to `main`
without merging the vendor files.
...
Findings were verified by reading the four vendored bundles on `chore/vendor-multiplayer-bundles`,
not just the experiment files. Where this document contradicts a header comment in the HTML, this
document is right; the corresponding comment fixes live with the vendor-baseline/code branches until
...
1. **`SEED` breaks silently — highest-severity item in this section.** `SEED` reads `?mp_session=`
   ...
   the LocalAdapter constructor* — confirmed from the vendored local adapter in PR #1, which writes
...
- **The PR #1 vendor pinning is correct and internally consistent.** Bundles built from `69c0d7b`,
  loaded from commit `5046bf0`, and both HTML files in that packet pin `5046bf0`. The stale-pin
  hazard documented there is real but not currently triggered.

> **Do not "fix" the PR #1 `vendor/` pins back to npm.** The published `0.1.0` builds predate the
> `jsPsych.multiplayer` namespace migration and do not work with the pinned core.
